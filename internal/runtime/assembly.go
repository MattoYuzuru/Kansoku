...
// Public listeners are created only by Run after every step succeeds.
func NewAppliance(ctx context.Context, config Config, secrets Secrets, integrityRunner IntegrityRunner) (*Appliance, error) {
	...
	if err != nil {
		return nil, errors.New("mirror_store_open_failed")
	}
	...
	ingressHTTP, err := observability.NewIngressHTTPHandler(ingressGuard, ingestor, receiver)
	if err != nil {
